console.log('')
console.log('-------------------------------')
console.log('  Скрипт Bot Fenix запущен.')
console.log('  Разработчик: Senate')
console.log('  The best bot')
...
bot(`Привет, отправь «помощь» что бы получить список моих команд. 📚

Беседа с ботом: https://vk.me/join/AJQ1d51sHA2ZqZfhqm0MSjsp`, 
		{
...
cmd.hear(/^(?:донат)$/i, async (message, bot) => {
	return bot(`узнать список товаров можно в нашей группе вконтакте ✅`);
});
...
cmd.hear(/^(?:работа)$/i, async (message, bot) => {
	...
	🔹 1. Дворник [1 уровень] - зарплата ~12.500$
	🔹 2. Сантехник [3 уровень] - зарплата ~22.500$
	🔹 3. Электрик [5 уровень] - зарплата ~25.000$
	🔹 4. Слесарь [8 уровень] - зарплата ~30.000$
	🔹 5. Юрист [10 уровень] - зарплата ~45.000$
	🔹 6. Бухгалтер [14 уровень] - зарплата ~55.000$
	🔹 7. Бармен [22 уровень] - зарплата ~60.000$
	🔹 8. Администратор [25 уровень] - зарплата ~75.000$
	🔹 9. Программист [49 уровень] - зарплата ~100.000$
...
Для покупки бизнеса введите «Бизнесы»`);
...
cmd.hear(/^(?:ферма|🔋 Ферма|@destinybot 🔋 Ферма)$/i, async (message, bot) => {
	...
	if(!message.user.misc.farm) return bot(`у Вас нет ферм. ${smileerror2}`);
